...
namespace EzySlice {
	...
	public sealed class SlicedHull {
		...
		public GameObject CreateUpperHull(GameObject original, Material crossSectionMat) {
			bool hasMeshRenderer = (original.GetComponent<MeshRenderer>() != null);

			GameObject newObject = CreateUpperHull(hasMeshRenderer);
			...
			if (newObject != null) {
				...
				Material[] shared = new Material[0];
		...
		public GameObject CreateLowerHull(GameObject original, Material crossSectionMat) {
			...
			GameObject newObject = CreateLowerHull(hasMeshRenderer);
         ...
         */
        public GameObject CreateUpperHull(bool hasMeshRenderer) {
            return CreateEmptyObject("Upper_Hull", upper_hull, hasMeshRenderer);
        }
		 ...
		 */
		public GameObject CreateLowerHull(bool hasMeshRenderer) {
			return CreateEmptyObject("Lower_Hull", lower_hull, hasMeshRenderer);
		}
		 ...
		 */
		private static GameObject CreateEmptyObject(string name, Mesh hull, bool hasMeshRenderer) {
			if (hull == null) {
				return null;
			}
